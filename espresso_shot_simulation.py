...
outer_coffee_thickness = 0.0079  # Thickness of the outer water layer (m)   originally 0.0079 / 0.0059
...
r_aw = np.linspace(R + d, R + d + outer_coffee_thickness, 790)  # Radial points in the outer water layer
...
r = np.concatenate([r_w, r_s, r_aw, r_air])
...
initial_temp = np.concatenate([last_temp_profile[:1500],
                                 np.full_like(r_aw, T_outer_coffee_initial), 
...
for i in range(0, len(time_points), max(1, len(time_points) // 10)):
    plt.plot(np.concatenate((r_w, r_s, r_aw, r_air))*1000, solution.y[:, i]-273, label=f't={time_points[i]:.1f}s')
